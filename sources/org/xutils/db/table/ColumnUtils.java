...
public final class ColumnUtils {
    ...
    public static boolean isTextColumnDbType(Object value) {
        ...
        if (value == null || (converter = ColumnConverterFactory.getColumnConverter(value.getClass())) == null || !ColumnDbType.TEXT.equals(converter.getColumnDbType())) {
            return false;
        }
        return true;
    }
    ...
    private static Method findBooleanSetMethod(Class<?> entityType, String fieldName, Class<?> fieldType) {
        ...
        if (!fieldName.startsWith("is") || fieldName.length() <= 2) {
            ...
            if (fieldName.length() > 3) {
                methodName = methodName + fieldName.substring(3);
            }
        }
        try {
