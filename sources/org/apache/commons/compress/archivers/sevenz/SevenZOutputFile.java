...
public class SevenZOutputFile implements Closeable {
    ...
    public SevenZOutputFile(SeekableByteChannel channel2) throws IOException {
    ...
    public void closeArchiveEntry() throws IOException {
        ...
        SevenZArchiveEntry entry = list.get(list.size() - 1);
        if (this.fileBytesWritten > 0) {
            ...
            if (countingOutputStreamArr != null) {
                long[] sizes = new long[countingOutputStreamArr.length];
                int i = 0;
                while (true) {
                    ...
                    sizes[i] = countingOutputStreamArr2[i].getBytesWritten();
                    i++;
                }
    ...
    public void finish() throws IOException {
        if (!this.finished) {
            ...
            bb.putInt(0);
            bb.putLong(headerPosition - 32).putLong(4294967295L & ((long) headerBytes.length)).putInt((int) crc322.getValue());
            crc322.reset();
            crc322.update(bb.array(), SevenZFile.sevenZSignature.length + 6, 20);
            bb.putInt(SevenZFile.sevenZSignature.length + 2, (int) crc322.getValue());
            bb.flip();
    ...
    private CountingOutputStream setupFileOutputStream() throws IOException {
        if (!this.files.isEmpty()) {
            ...
            boolean first = true;
            List<SevenZArchiveEntry> list = this.files;
            for (SevenZMethodConfiguration m : getContentMethods(list.get(list.size() - 1))) {
                if (!first) {
    ...
    private class OutputStreamWrapper extends OutputStream {
        ...
        public void write(byte[] b, int off, int len) throws IOException {
            ...
            long unused = sevenZOutputFile.fileBytesWritten = sevenZOutputFile.fileBytesWritten + ((long) len);
        }
